...
public class CTCommand extends CommandBase {
    ...
    private void simulateChat(String[] args) {
        ...
        ClientChatReceivedEvent event = new ClientChatReceivedEvent(ChatType.CHAT, new TextComponentString(toSend.toString().trim()));
        CTJS.getInstance().getChatListener().onReceiveChat(event);

        if (!event.isCanceled()) {
            MinecraftVars.getPlayer().sendMessage(new TextComponentString(ChatLib.addColor(toSend.toString().trim())));
        }
    }
    ...
    private void dumpChat(int lines) {
        ...
        for (int i = 0; i < lines; i++) {
            ...
            TextComponentString cct = new TextComponentString(msg);

            cct.setStyle(new Style()
                    .setClickEvent(
                            new ClickEvent(ClickEvent.Action.getValueByCanonicalName("run_command"), "/ct copy " + msg))
                    .setHoverEvent(
                            new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString("§eClick here to copy this message."))));
