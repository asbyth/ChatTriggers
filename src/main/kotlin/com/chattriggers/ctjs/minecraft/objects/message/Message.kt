...
class Message {
    ...
    private var messageParts = mutableListOf<ITextComponent>()
    ...
    constructor(component: ITextComponent) {
        if (component.siblings.isEmpty()) {
            this.messageParts.add(StringTextComponent("").appendSibling(component))
        } else {
            this.messageParts.addAll(component.siblings.map { StringTextComponent("").appendSibling(it) })
        }
    }
    ...
    constructor(messageParts: ArrayList<Any>) {
        this.messageParts.addAll(messageParts.map {
            when (it) {
                is String -> StringTextComponent(it)
                is TextComponent -> it
                else -> return
            }
        })
    ...
    fun setTextComponent(index: Int, component: Any) = apply {
        when (component) {
            is String -> this.messageParts[index] = StringTextComponent(component)
    ...
    fun addTextComponent(component: Any) = apply {
        when (component) {
            is String -> this.messageParts.add(StringTextComponent(component))
    ...
    fun addTextComponent(index: Int, component: Any) = apply {
        when (component) {
            is String -> this.messageParts.add(index, StringTextComponent(component))
    ...
    private fun parseMessage() {
        this.chatMessage = StringTextComponent("")

        for (messagePart in this.messageParts) {
            this.chatMessage.appendSibling(messagePart)
        }
    }
}