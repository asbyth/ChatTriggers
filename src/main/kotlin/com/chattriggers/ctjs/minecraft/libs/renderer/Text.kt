...
class Text @JvmOverloads constructor(private var string: String, private var x: Float = 0f, private var y: Float = 0f) {
    ...
    fun draw(x: Float? = null, y: Float? = null) = apply {
        ...
        if (this.width > 0) {
            ...
            this.lines.forEach {
                if (shadow) {
                    Renderer.getFontRenderer().drawStringWithShadow(it, getXAlign(it, x
                        ?: this.x), yHolder / this.scale, this.color.toInt())
                } else {
                    Renderer.getFontRenderer().drawString(it, getXAlign(it, x
                        ?: this.x), yHolder / this.scale, this.color.toInt())
                }
