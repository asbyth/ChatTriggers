...
object ChatLib {
    ...
    fun say(text: String) = Player.getPlayer()?.sendChatMessage(text) ?:
        throw IllegalStateException("Attempt to call ChatLib.say(...) without a valid client player")
    ...
    fun command(text: String, clientSide: Boolean = false) {
        // TODO: Is the leading slack necessary here?
        if (clientSide) CommandHandler.commandDispatcher.execute("/$text", Player.getPlayer()?.commandSource)
        else say("/$text")
    }
