...
object Renderer {
    ...
    @JvmStatic
    fun getRenderManager(): EntityRendererManager {
    ...
    fun drawString(text: String, x: Float, y: Float) {
        ...
        if (text.contains("\n")) {
            ...
            ChatLib.addColor(text).split("\n").forEach {
                fr.drawString(it, x, newY, colorized?.toInt() ?: WHITE.toInt())
        ...
        fr.drawString(ChatLib.addColor(text), x, y, colorized?.toInt() ?: 0xffffffff.toInt())
        finishDraw()
    }

    @JvmStatic
    fun drawStringWithShadow(text: String, x: Float, y: Float) {
        getFontRenderer().drawStringWithShadow(ChatLib.addColor(text), x, y, colorized?.toInt() ?: 0xffffffff.toInt())
        finishDraw()
    }
    ...
    fun drawPlayer(player: Any, x: Int, y: Int, rotate: Boolean = false) {
        ...
        var ent: LivingEntity = Player.getPlayer()!!
        ...
        GL11.glRotatef(-45.0f, 0.0f, 1.0f, 0.0f)
        GL11.glRotatef(-atan((mouseY / 40.0f).toDouble()).toFloat() * 20.0f, 1.0f, 0.0f, 0.0f)
        if (!rotate) {
    ...
    object screen {
        @JvmStatic
        fun getWidth(): Int = Client.mc.mainWindow.scaledWidth

        @JvmStatic
        fun getHeight(): Int = Client.mc.mainWindow.scaledHeight

        @JvmStatic
        fun getScale(): Double = Client.mc.mainWindow.guiScaleFactor
    }
}