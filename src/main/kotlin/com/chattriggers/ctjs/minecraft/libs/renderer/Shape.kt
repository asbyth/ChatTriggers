...
class Shape(private var color: Int) {
    ...
    fun getVertexes(): List<Vec2f> = this.vertexes
    fun addVertex(x: Float, y: Float) = apply { this.vertexes.add(Vec2f(x, y)) }
    fun insertVertex(index: Int, x: Float, y: Float) = apply { this.vertexes.add(index, Vec2f(x, y)) }
    ...
    fun setLine(x1: Float, y1: Float, x2: Float, y2: Float, thickness: Float) = apply {
        ...
        this.vertexes.add(Vec2f(x1 + i, y1 + j))
        this.vertexes.add(Vec2f(x2 + i, y2 + j))
        this.vertexes.add(Vec2f(x2 - i, y2 - j))
        this.vertexes.add(Vec2f(x1 - i, y1 - j))
    ...
    fun setCircle(x: Float, y: Float, radius: Float, steps: Int) = apply {
        ...
        for (i in 0..steps) {
            this.vertexes.add(Vec2f(x, y))
            this.vertexes.add(Vec2f(circleX * radius + x, circleY * radius + y))
            xHolder = circleX
