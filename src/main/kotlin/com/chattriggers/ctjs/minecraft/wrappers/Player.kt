...
object Player {
     ...
     */
    @JvmStatic
    fun getPlayer(): ClientPlayerEntity? = Client.getMinecraft().player
     ...
     */
    @JvmStatic
    fun getMotionX(): Double = getPlayer()?.motion?.x ?: 0.0
     ...
     */
    @JvmStatic
    fun getMotionY(): Double = getPlayer()?.motion?.y ?: 0.0
     ...
     */
    @JvmStatic
    fun getMotionZ(): Double = getPlayer()?.motion?.z ?: 0.0
     ...
     */
    @JvmStatic
    fun getPitch(): Float = MathHelper.wrapDegrees(getPlayer()?.rotationPitch ?: 0f)
     ...
     */
    @JvmStatic
    fun getYaw(): Float = MathHelper.wrapDegrees(getPlayer()?.rotationYaw ?: 0f)
    ...
    @JvmStatic
    fun isSneaking(): Boolean = getPlayer()?.isShiftKeyDown ?: false
    ...
    fun lookingAt(): Any {
        ...
        return when (mop) {
            is BlockRayTraceResult -> {
                val block = Block(world.getBlockState(mop.pos).block)
                    .setBlockPos(mop.pos)
                    .setFace(BlockFace(mop.face))

                val subBlock = block.block

                if (subBlock is AbstractSignBlock) Sign(subBlock) else block
            }
    ...
    private fun getPlayerName(networkPlayerInfoIn: NetworkPlayerInfo): String? {
        return if (networkPlayerInfoIn.displayName != null)
            ...
            networkPlayerInfoIn.playerTeam?.format(networkPlayerInfoIn.displayName!!)?.formattedText
    }
