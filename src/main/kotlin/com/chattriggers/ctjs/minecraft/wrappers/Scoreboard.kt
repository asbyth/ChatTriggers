...
object Scoreboard {
    ...
    fun setShouldRender(shouldRender: Boolean) {
        ForgeIngameGui.renderObjective = shouldRender
    }

    @JvmStatic
    fun getShouldRender() = ForgeIngameGui.renderObjective
    ...
    private fun updateNames() {
        ...
        try {
            val scoreboard = World.getWorld()?.scoreboard ?: return
            val sidebarObjective = scoreboard.getObjectiveInDisplaySlot(1)
            scoreboardTitle = sidebarObjective?.displayName?.formattedText ?: return
    ...
    class Score(val score: net.minecraft.scoreboard.Score) {
         ...
         */
        fun getName(): String = ScorePlayerTeam.formatMemberName(
            World.getWorld()!!.scoreboard.getPlayersTeam(score.playerName),
            StringTextComponent(score.playerName)
